...
export function renderPlayerProfile(target, context) {
  ...
  visual.append(
    createCoachNote(
      "Coach tip: plus tu joues de minutes, plus tes statistiques racontent ta vraie regularite."
    )
  );
  ...
  metricsSection.append(el("h3", { text: "Stats cles" }));

  const allMetrics = getMetricEntries(player);
  const primary = allMetrics.slice(0, 6);
  const extra = allMetrics.slice(6);
