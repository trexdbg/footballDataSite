...
function renderRow(player, context) {
  ...
  nameTd.append(nationalityLine);

  const clubTd = document.createElement("td");
  clubTd.append(highlightText(player.club?.name || "—", context.searchQuery));
...
function renderCard(player, context) {
  ...
  card.append(topRow);
  card.append(el("p", { className: "muted", text: `${player.position} • ${player.club?.name || "—"}` }));
  card.append(el("p", { text: `Minutes: ${formatNumber(player.stats?.minutes, 0)} | Saison: ${player.seasonKey}` }));
  ...
  const compareBtn = el("button", {
    ...
    attrs: { type: "button", "aria-label": `Ajouter ${player.name} à la comparaison` },
  });
  compareBtn.addEventListener("click", () => context.actions.addPlayerToCompare(player.slug));
...
export function renderPlayersTableView(target, context) {
  ...
  page.append(
    createCoachNote(
      "Coach tip: 80% de passes réussies veut dire que sur 10 passes, environ 8 arrivent au bon coéquipier."
    )
  );
  ...
  const infoLine = el("p", {
    ...
    text: `${sorted.length} joueur(s) affiché(s)`,
  });
    ...
    page.append(
      el("p", {
        ...
        text:
          ...
          "Aucun joueur trouvé avec ces filtres. Essaie d'enlever un filtre ou de réduire les minutes minimum.",
      })
    );
    target.append(page);
    return;
