...
const SORTABLE_COLUMNS = [
  ...
  { key: "passAccuracy", label: "Precision passe" },
  { key: "duelsWonRate", label: "Taux duels" },
];
...
function createSortButton(column, sortState, onSort) {
  ...
  const directionMark = isActive ? (sortState.dir === "asc" ? "ASC" : "DESC") : "";
...
function renderCard(player, context) {
  ...
  let quickStats = metricsForPosition(player, { limit: 3, preferPer90: false });
  if (quickStats.length < 3) {
    quickStats = metricsForPosition(player, { limit: 3, preferPer90: true });
  }
  const list = el("ul", { className: "stats-list" });
  quickStats.forEach((entry) => {
    list.append(el("li", { text: `${metricLabel(entry.key)}: ${metricValueText(entry)}` }));
  });
  card.append(list);
...
export function renderPlayersTableView(target, context) {
  ...
  page.append(
    createCoachNote(
      "Coach tip: lis les KPI selon le poste pour eviter les comparaisons trompeuses."
    )
  );
